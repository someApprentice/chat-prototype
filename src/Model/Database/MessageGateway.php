...
class MessageGateway extends UserGateway
{
    ...
    public function getNewMessages($user, $conference, $since)
    {
        ...
        $query = $pdo->prepare("SELECT mbox.messageID, messages.author, messages.receiver, users.name, messages.date, .messages.content FROM mbox INNER JOIN messages ON mbox.messageID = messages.id INNER JOIN users ON messages.author = users.id WHERE (user=:user AND conference=:conference) AND messages.author != :user AND messages.date > :since ORDER BY messages.date ASC");
        $query->bindValue(':user', $user);
        $query->bindValue(':conference', $conference);
